...
module.exports.deaggregateSync = (kinesisRecord, computeChecksums, afterRecordCallback) => {
  const userRecords = [];

  // use the async deaggregation interface, and accumulate user records into
  // the userRecords array
  exports.deaggregate(kinesisRecord, computeChecksums, async (err, userRecord) => {
    if (err) {
      await afterRecordCallback(err);
    } else {
      userRecords.push(userRecord);
    }
  }, async (err) => {
    await afterRecordCallback(err, userRecords);
  });
};
